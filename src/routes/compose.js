...
const listProjects = db.prepare(`
  SELECT cp.id, cp.name, cp.description, cp.created_at, cp.updated_at,
         COUNT(CASE WHEN csl.host_id IS NOT NULL THEN 1 END) AS linked_count
...
router.get('/:id', requireAuth, (req, res) => {
  const project = getProject.get(parseInt(req.params.id, 10));
  if (!project) return res.status(404).json({ error: 'Compose project not found' });
  res.json({ ...project, links: getLinks.all(project.id) });
});
